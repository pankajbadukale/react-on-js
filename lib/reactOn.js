    ...
    while (this._stream) {
      ...
      let nextState = stream.find((val) => {
        if (secondResult.state == val.state) {
          return (this._back || this._stream.childState == undefined || this._stream.childState.length == 0 || this._stream.childState.indexOf(secondResult.state) > -1 );
        } else {
          return false;
        }
      });
      if(nextState){      
        if(!this._back)  {
          this.stackStream.push(Object.assign({},this._stream));   
        }
